...
//----------------------------------------------------------------------------
IntrRay3Box3f::IntrRay3Box3f(const Ray3f& rRay, const Box3f& rBox)
    :
    m_pRay(&rRay),
    m_pBox(&rBox)
{
...
//----------------------------------------------------------------------------
const Ray3f& IntrRay3Box3f::GetRay() const
{
    return *m_pRay;
}
//----------------------------------------------------------------------------
const Box3f& IntrRay3Box3f::GetBox() const
{
    return *m_pBox;
}
//----------------------------------------------------------------------------
bool IntrRay3Box3f::Test()
{
    ...
    Vector3f vec3fDiff = m_pRay->Origin - m_pBox->Center;

    afWdU[0] = m_pRay->Direction.Dot(m_pBox->Axis[0]);
    afAWdU[0] = Math<float>::FAbs(afWdU[0]);
    afDdU[0] = vec3fDiff.Dot(m_pBox->Axis[0]);
    afADdU[0] = Math<float>::FAbs(afDdU[0]);
    ...
    afAWdU[1] = Math<float>::FAbs(afWdU[1]);
    afDdU[1] = vec3fDiff.Dot(m_pBox->Axis[1]);
    afADdU[1] = Math<float>::FAbs(afDdU[1]);
    ...
    afAWdU[2] = Math<float>::FAbs(afWdU[2]);
    afDdU[2] = vec3fDiff.Dot(m_pBox->Axis[2]);
    afADdU[2] = Math<float>::FAbs(afDdU[2]);
    if( afADdU[2] > m_pBox->Extent[2] && afDdU[2]*afWdU[2] >= 0.0f )
    {
        return false;
    }

    Vector3f vec3fWxD = m_pRay->Direction.Cross(vec3fDiff);
...
{
    float fT0 = 0.0f, fT1 = Math<float>::MAX_REAL;
...
//----------------------------------------------------------------------------
const Vector3f& IntrRay3Box3f::GetPoint(int i) const
{
