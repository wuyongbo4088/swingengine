...
int SEWindowApplication::Main(int, char**)
{
    ...
    // 创建渲染器.
    pTheApp->SetRenderer(SE_NEW SEDX10Renderer(hWnd, m_eFormat, m_eDepth,
    ...
    int iXSize, int iYSize)
{
    ...
    if( !pControl )
    {
        pControl = SE_NEW SETreeControl(this, 0, GetModuleHandle(0), 0, 
            pScene, iXPos, iYPos, iXSize, iYSize);
